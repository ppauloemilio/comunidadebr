...
export async function authMiddleware(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const user = (await db.prepare('SELECT * FROM users WHERE id = ?').get(payload.sub)) as UserRow | undefined;
...
export async function createNotification(
  ...
  const db = await getDb();
  const actor = await db.prepare('SELECT * FROM users WHERE id = ?').get(actorId) as UserRow;
  await db.prepare(
